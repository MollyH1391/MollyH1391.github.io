...
changeBtn.setAttribute('data-bs-target', '#contactModal')
const changeBtn_icon = document.createElement('i')
changeBtn_icon.classList.add('fas', 'fa-phone', 'rounded-circle', 'text_blue', 'bg_white', 'd-flex', 'justify-content-center', 'align-items-center')
const changeBtn_p = document.createElement('p')
changeBtn_p.classList.add('mt-2', 'text_white')
changeBtn_p.innerText = "聯絡我"
...
function getSearchAreaInfo() {
    ...
    getSearchAreaPage(city, district, road,brandName)
}

function getSearchOrderedInfo() {
    if (isLogin == 'True') {
        getSearchOrderedPage(brandName)
    } else if (isLogin == 'False') {
        window.location.href = '/Member/Login'
    }
}
function getSearchNearInfo() {
    getSearchNearbyPage(brandName)
}


////常用地址
//isCheck ? finddisCheckInput.setAttribute('checked', '') : ('', '')
//finddisCheckInput.onclick = function () {
//    finddisCheckInput.forEach(x => {
//        if (x.checked) {
//            x.isCheck = true
